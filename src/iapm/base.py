...
def echo(msg, loglevel, color=False, level=0):
    ...
    # An Critical, or even worse, always can stop a operation.

    # Critical/Fatal: WORSE than Error, in case of it means errors must to be handled (For example, operation failed when IAPM is working on action:update/install and stage:update, means system is in a inconsistent status).
