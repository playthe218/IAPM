...
### 偷懒 ###
写代码写到一半突然想摆烂了？我懂，我也懒，IAPM 刚开始也只是一个我为了学习 Python 建立的项目。 <br>
虽然不建议提交未完成的部分，但如果你实在有什么事情写不动了(学业工作也行，凿代码凿累了也行，突然想去玩游戏了也行):  <br>
1. 看一眼代码，确保就算你后续也不维护它了，其他贡献者也方便接手。
2. 在你未完成的代码标注一句注释，像我自己就写了个 `# PLAYTHE218!!! Look your mess!!!` 。
3. 提交 PR 的时候注明哪个文件有你未完成的部分。
   
注意，如果你未完成的代码部分，修改的目的是性能优化、修复 bugs/漏洞 之类的，那就不要提交这些代码，毕竟没有人希望看见一个本来只是有点小问题但还能用的功能在这个 PR 之后就罢工了，非要交就在相关文件后面加 `-undone` 。

### 哪些 PR 会被拒绝 ###
以下记录了通常会被拒绝的 PR ，请避免相同情况： <br>
1. "热心"的格式化: 只是把代码丢进 black 格式化一遍，并没有其他实际的功能、修复或优化性改动，然后交了 PR 。在每次发布 Release 之前，主要维护者<del>TNND, 怎么还是我  ——PLAYThe218</del>会自行 black 所有代码并进行预期验证。
2. 过度优化: 为了提高性能或精简代码大小(甚至只是为了省一个换行符)，严重忽视了可读性；最坏的情况下，这样的过度优化或许可读性很低，而其目的也未能达成:
    ...
    ```
3. 我导入了 100 个包：导入了大量的 Python 包，甚至包括了绝大部分 Linux 发行版的最小安装甚至基础安装不会安装的。
...
6. Python和C++还是太逊了，我要用Rust：不言而喻，你个活爹。不要和我说什么 "阿拉，Rust 安全多了" ，软件包管理器最大的危险其实是 keyring 污染。
